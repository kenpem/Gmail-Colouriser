...
** colour tags depending on the event titles, creators and attendees.
**
...
** TODO: move the calendar selection and string/colour matching to an options section. I hate having noise that like hard-coded. (can I add an extension to GMail to do that through a nice config window?)
...
** DONE: allow colour allocation for appointments which are just for me and one other person.
*/
...
function ColourEvent(e) {
  ...
  while ((step < 999) && (!done)) {
    if (!done) switch (step) {
      ...
      // Note that colours are applied in the priority order below. So if a meeting title is "Manage beer", it's the "Manage" that will be applied.
      ...
      case 3 : done = setColour(e,MatchColourToEvent(e,CalendarApp.EventColor.RED       ,title,"Manage","Leader","Delivery","Town Hall"));                 break;
      ...
      case 7 : if (e.getGuestList().length == 0) // no guests, so this is a me-only appointment
               ...
               break;
               // note that the case below somtimes returns an incorrect count of one if there's only one distribution list (which can be many participants). I can live with that.
      case 8 : if (e.getGuestList().length == 1) // one guest, so this is a one-to-one meeting (not the same as a formal 1:1 as handled above)
